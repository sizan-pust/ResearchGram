...
type RequestKind = "research" | "profile";
...
export default function RequestsPage() {
  ...
  const loadRequests = async () => {
    ...
    if (researchReceivedError) {
      console.log("FETCH RECEIVED RESEARCH REQUESTS ERROR:", researchReceivedError);
    }
    ...
    if (profileReceivedError) {
      console.log("FETCH RECEIVED PROFILE REQUESTS ERROR:", profileReceivedError);
    }
    ...
    const normalizedProfileReceived: UnifiedRequestView[] =
      profileReceived.map((request) => ({
        id: request.id,
        kind: "profile",
        content_id: null,
    ...
    setReceivedRequests(
      sortByNewest([...normalizedResearchReceived, ...normalizedProfileReceived]),
    );
    setSentRequests(
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 border-b border-gray-100 pb-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-500">
                Manage collaboration invitations, profile connections, and academic networking requests.
              </p>
            </div>
                ...
                >
                  ...
                  {request.kind === "profile" && (
                    <div className="mt-5 rounded-2xl bg-purple-50 p-4">
                      ...
                      <p className="mt-2 text-sm leading-6 text-gray-600">
                        Connections help researchers discover each other, build academic networks,
                        and collaborate beyond individual posts.
                      </p>

                      <button
                        onClick={() =>
                          router.push(`/researchers/${request.otherProfile?.id}`)
                        }
                        className="mt-3 rounded-full bg-white px-4 py-2 text-xs font-semibold text-purple-700 transition hover:bg-purple-100"
                      >
                        View Profile
                      </button>
                    </div>
                  ...
                  {activeTab === "received" && request.status === "pending" && (
                    <div className="mt-5 flex flex-wrap justify-end gap-3 border-t border-gray-100 pt-4">
                      <button
                        onClick={() =>
                          handleUpdateStatus(request, "declined")
                        }
                        disabled={updatingId === request.id}
                        className="rounded-full bg-red-50 px-5 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                      ...
                      <button
                        onClick={() =>
                          handleUpdateStatus(request, "accepted")
                        }
                        disabled={updatingId === request.id}
                        className="rounded-full bg-green-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-60"
                      >
